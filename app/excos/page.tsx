...
import ExcoUpdateModal from '../excos/modals/ExcoUpdateModal';
...
export default function ExcosPage() {
  ...
  const handleUpdate = async (updatedData: Executive): Promise<void> => {
    ...
    setExcosData(prev => ({
      ...
      [selectedYear]: prev[selectedYear].map(exco => 
        exco.id === updatedData.id ? updatedData : exco
      )
    }));
  ...
  return (
    <>
      {/* Only show main content when modal is NOT open */}
      {!isModalOpen ? (
        <main>
          {/* Header */}
        ...
        /* Empty div when modal is open to maintain layout */
